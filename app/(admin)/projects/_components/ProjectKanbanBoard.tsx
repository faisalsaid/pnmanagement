  ...
  return (
    <div className="flex gap-2">
      {columns.map((colum) => (
        <div
          key={colum.title}
          className="min-w-52 border rounded-md p-2 w-full space-y-4 bg-accent"
        >
          <div className="flex gap-2 text-sm">
            <p>{colum.title}</p>{' '}
            <p className="bg-muted px-2 rounded-sm ">{colum.taskItem}</p>
          </div>
          {Array.from({ length: colum.taskItem }, (_, i) => (
            <Card key={i} className="rounded-sm p-2 gap-4 ">
              ...
              <div className="text-xs flex items-center gap-2 px-2 py-1 bg-green-400/20 max-w-fit rounded-sm">
                <Flag size={12} />
